...
static const double minConfidenceThreshold = 0.5; // Require better confidence
...
+ (NSDictionary * _Nullable)detectBallInFrame:(UIImage *)frame {
    ...
    if (tracker.position.x >= 0 && tracker.position.y >= 0) {
        NSLog(@"[OpenCV] Real-time: Ball detected at (%d, %d)", tracker.position.x, tracker.position.y);
...
static cv::Point detectBallByShape(const cv::Mat& frame) {
    ...
    if (!circles.empty()) {
        ...
        return cv::Point(cvRound(circle[0]), cvRound(circle[1]));
    }
    return cv::Point(-1, -1);
}
